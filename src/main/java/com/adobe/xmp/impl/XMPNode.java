...
/**
 * A node in the internally XMP tree, which can be a schema node, a property node, an array node, an
 * array item, a struct node or a qualifier node (without '?').
 * 
 * Possible improvements:
 * 
 * 1. The kind Node of node might be better represented by a class-hierarchy of different nodes. 2.
 * The array type should be an enum 3. isImplicitNode should be removed completely and replaced by
 * return values of fi. 4. hasLanguage, hasType should be automatically maintained by XMPNode
 * 
 * @since 21.02.2006
 */
class XMPNode implements Comparable<Object> {
   ...
   *        <em>Note:</em> The node children are indexed from [1..size]! An index of size + 1
   *        appends a node.
   * @param node an XMPNode
   * @throws XMPException
   */
   ...
   * 
   * @param index the index of the node that will be replaced. <em>Note:</em> The node children are
   *        indexed from [1..size]!
  ...
  /**
   * Removes the children list if this node has no children anymore; checks if the provided node is
   * a schema node and doesn't have any children anymore, its deleted.
   */
   ...
   * @return Returns a qualifier <code>XMPNode</code> if node has been found, <code>null</code>
   *         otherwise.
   */
  ...
  /**
   * @return Returns an iterator for the children. <em>Note:</em> take care to use it.remove(), as
   *         the flag are not adjusted in that case.
   */
  ...
  /**
   * @return Returns an iterator for the qualifier. <em>Note:</em> take care to use it.remove(), as
   *         the flag are not adjusted in that case.
   */
  ...
  /**
   * Performs a <b>deep clone</b> of the complete subtree (children and qualifier )into and add it
   ...
   * <li>Starting at the root node the children and qualifier are sorted recursively, which the
   * following exceptions.
   * <li>Sorting will not be used for arrays.
   * <li>Within qualifier "xml:lang" and/or "rdf:type" stay at the top in that order, all others are
   * sorted.
   * </ul>
   */
  public void sort() {
    ...
    if (hasQualifier()) {
      ...
      int sortFrom = 0;
      while (quals.length > sortFrom && (XMPConst.XML_LANG.equals(quals[sortFrom].getName()) || "rdf:type".equals(quals[sortFrom].getName()))) {
        quals[sortFrom].sort();
        sortFrom++;
      }
  ...
  /**
   * Dumps this node and its qualifier and children recursively. <em>Note:</em> It creats empty
   * options on every node.
   * 
  ...
  private void dumpNode(StringBuffer result, boolean recursive, int indent, int index) {
    ...
    if (recursive && hasQualifier()) {
      XMPNode[] quals = (XMPNode[]) getQualifier().toArray(new XMPNode[getQualifierLength()]);
      int i = 0;
      while (quals.length > i && (XMPConst.XML_LANG.equals(quals[i].getName()) || "rdf:type".equals(quals[i].getName()))) {
        i++;
      }
  ...
  /**
   * <em>Note:</em> This method should always be called when accessing 'children' to be sure that
   * its initialized.
   * 
  ...
  /**
   * Sets the parent node, this is solely done by <code>addChild(...)</code> and
   * <code>addQualifier()</code>.
   * 
